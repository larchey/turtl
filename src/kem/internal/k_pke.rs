...
pub(crate) fn decode_private_key(
    ...
    let mut h = [0u8; 32];
    h.copy_from_slice(
        &private_key[384 * parameter_set.k() + pk_size..384 * parameter_set.k() + pk_size + 32],
    );

    let mut z = [0u8; 32];
    z.copy_from_slice(
        &private_key
            [384 * parameter_set.k() + pk_size + 32..384 * parameter_set.k() + pk_size + 64],
    );
...
fn compress(poly: &Polynomial, d: usize) -> Result<Polynomial> {
    ...
    for i in 0..256 {
        ...
        let compressed =
            ((((1 << d) as i64 * value as i64) + (q / 2) as i64) / q as i64) % (1 << d) as i64;
