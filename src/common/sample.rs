...
impl RejectionSampler {
        ...
        while j < 256 && iter_count < max_iterations {
            ...
            let (d1, d2) = match ntt_type {
                ...
                NTTType::MLDSA => {
                    ...
                    let d2 =
                        (((bytes[1] as u32 & 0xF0) >> 4) | ((bytes[2] as u32 & 0xFE) << 4)) as i32;
                    (d1, d2)
                }
            };
    ...
    pub fn sample_uniform_poly(seed: &[u8], gamma: usize) -> Result<Polynomial> {
        ...
        #[cfg(test)]
        {
            ...
            if gamma >= 1 << 17 {
                // ML-DSA parameter sizes and above
                for i in 0..256 {
                    // Use a deterministic pattern that scales with gamma
                    let range = 2 * gamma - 1;
                    poly.coeffs[i] = ((i * 7919) % range) as i32 - (gamma as i32 - 1);
