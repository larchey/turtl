...
use turtl::kem::{self, KeyPair, ParameterSet};
...
fn bench_kem512_encapsulate(c: &mut Criterion) {
    ...
    c.bench_function("ML-KEM-512 Encapsulate", |b| {
        b.iter(|| {
            let (ciphertext, shared_secret) = kem::encapsulate(black_box(&public_key)).unwrap();
...
fn bench_kem512_decapsulate(c: &mut Criterion) {
    ...
    c.bench_function("ML-KEM-512 Decapsulate", |b| {
        b.iter(|| {
            let shared_secret =
                kem::decapsulate(black_box(&private_key), black_box(&ciphertext)).unwrap();
...
fn bench_all_parameter_sets(c: &mut Criterion) {
    ...
    for param_set in [
        ...
        group.bench_with_input(
            BenchmarkId::new("Encapsulate", name),
            &public_key,
            |b, pk| {
                b.iter(|| {
                    let (ciphertext, shared_secret) = kem::encapsulate(black_box(pk)).unwrap();
                    black_box((ciphertext, shared_secret))
                })
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, (sk, ct)| {
                b.iter(|| {
                    let shared_secret = kem::decapsulate(black_box(sk), black_box(ct)).unwrap();
                    black_box(shared_secret)
                })
            },
        );
