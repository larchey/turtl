...
fn test_ml_kem_parameter_validation() {
    ...
    assert!(
        param_set.validate().is_ok(),
        "Valid parameter set should pass validation"
    );
    ...
    assert_eq!(
        param_set.k(),
        2,
        "Matrix dimension k should be 2 for ML-KEM-512"
    );
    ...
    assert_eq!(
        param_set.d(),
        13,
        "d should be 13 for all ML-KEM parameter sets"
    );
    ...
    assert_eq!(
        ...
        32,
        "Shared secret size should be 32 bytes"
    );
    ...
    assert_eq!(
        param_set.k(),
        3,
        "Matrix dimension k should be 3 for ML-KEM-768"
    );
    ...
    assert_eq!(
        param_set.k(),
        4,
        "Matrix dimension k should be 4 for ML-KEM-1024"
    );
