...
fn test_constant_time_operations() {
    ...
    assert_eq!(
        x, 20,
        "ct_cmov should not change value when condition is false"
    );
    ...
    assert_eq!(
        result, 5,
        "ct_select should return first value when condition is true"
    );

    let result = constant_time::ct_select(5, 10, false);
    assert_eq!(
        result, 10,
        "ct_select should return second value when condition is false"
    );
    ...
    assert_eq!(
        a, 2,
        "ct_cswap should not swap values when condition is false"
    );
    assert_eq!(
        b, 1,
        "ct_cswap should not swap values when condition is false"
...
fn test_constant_time_comparisons() {
    ...
    assert!(
        constant_time::ct_eq_u32(100, 100),
        "Equal values should return true"
    );
    assert!(
        !constant_time::ct_eq_u32(100, 101),
        "Unequal values should return false"
    );

    // Test zero check
    assert!(
        constant_time::ct_is_zero_u32(0),
        "Zero value should return true"
    );
    assert!(
        !constant_time::ct_is_zero_u32(1),
        "Non-zero value should return false"
    );

    // Test byte operations
    assert!(
        constant_time::ct_is_zero_u8(0),
        "Zero byte should return true"
    );
    assert!(
        !constant_time::ct_is_zero_u8(255),
        "Non-zero byte should return false"
    );
}
...
fn test_slice_operations() {
    ...
    assert!(
        ...
        "Equal slices should return true"
    );
    assert!(
